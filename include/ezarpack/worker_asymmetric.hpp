...
/// @brief Main worker class for the general real eigenproblems.
///
/// This specialization of `arpack_worker` calls ARPACK-NG functions `dnaupd()`
/// and `dneupd()` to compute approximations to a few eigenpairs of a real
/// linear operator @f$ \hat O @f$ with respect to a real positive semi-definite
/// symmetric matrix @f$ \hat B @f$.
///
/// @note If the linear operator @f$ \hat O @f$ is symmetric with respect to
/// @f$ \hat B@f$, @ref arpack_worker<Symmetric, Backend>, then should be
/// used instead.
///
